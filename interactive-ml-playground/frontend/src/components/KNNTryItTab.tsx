...
import { apiClient, DataUploadResponse, KNNTrainResponse } from "@/lib/api";
...
export default function KNNTryItTab() {
  ...
  // Results state
  const [trainResult, setTrainResult] = useState<KNNTrainResponse | null>(null);
  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      const result = await apiClient.trainKNN(
        data,
        selectedFeatures,
        targetColumn,
        nNeighbors,
        ...
        <div className="bg-terminal-panel border-2 border-terminal-black p-6">
          ...
          <div className="mt-6 p-4 bg-terminal-black text-terminal-mint">
            <p className="font-mono text-xs">
              <span className="text-terminal-accent font-bold">TIP:</span> KNN REQUIRES NUMERIC FEATURES AND A CATEGORICAL TARGET COLUMN.
              THE IRIS DATASET IS A CLASSIC EXAMPLE FOR CLASSIFICATION.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-terminal-panel border-2 border-terminal-black p-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block font-mono text-xs font-bold uppercase tracking-terminal text-terminal-black mb-3">
                  NUMBER OF NEIGHBORS (K): {nNeighbors}
                </label>
                <input
                  type="range"
                  min="1"
                ...
                <p className="font-mono text-xs text-terminal-black/50 mt-1">
                  SMALL K = MORE SENSITIVE // LARGE K = SMOOTHER BOUNDARIES
                </p>
              </div>
          ...
          <div className="bg-terminal-accent/20 border-2 border-terminal-accent p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-mono text-xs text-terminal-black/70">
                K={trainResult.metrics.n_neighbors} | {trainResult.metrics.n_classes} CLASSES DETECTED
              </p>
            </div>
          </div>
