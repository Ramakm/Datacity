...
import { apiClient, DataUploadResponse, DecisionTreeTrainResponse } from "@/lib/api";
import CodeBlock from "./CodeBlock";

type Step = "upload" | "configure" | "results";

const SAMPLE_CSV = `sepal_length,sepal_width,petal_length,petal_width,species
...
export default function DecisionTreeTryItTab() {
  ...
  // Results state
  const [trainResult, setTrainResult] = useState<DecisionTreeTrainResponse | null>(null);
  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      const result = await apiClient.trainDecisionTree(
        data,
        selectedFeatures,
        targetColumn,
        maxDepth,
        ...
        <div className="bg-terminal-panel border-2 border-terminal-black p-6">
          ...
          <div className="mt-6 p-4 bg-terminal-black text-terminal-mint">
            <p className="font-mono text-xs">
              <span className="text-terminal-accent font-bold">TIP:</span> DECISION TREES WORK WITH NUMERIC FEATURES AND A CATEGORICAL TARGET.
              THEY CREATE INTERPRETABLE IF-ELSE RULES FOR CLASSIFICATION.
            </p>
          </div>
        </div>
      )}
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block font-mono text-xs font-bold uppercase tracking-terminal text-terminal-black mb-3">
                  SELECT FEATURE COLUMNS (NUMERIC INPUTS)
                </label>
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <input
                    ...
                    className="flex-1 max-w-xs accent-terminal-accent"
                  />
                  <button
                    onClick={() => setMaxDepth(maxDepth === null ? 10 : null)}
                    className={clsx(
                  ...
                  >
                    {maxDepth === null ? "LIMITED" : "UNLIMITED"}
                  </button>
                </div>
          ...
          <div className="bg-terminal-accent/20 border-2 border-terminal-accent p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-mono text-xs text-terminal-black/70">
                DEPTH={trainResult.metrics.actual_depth} | LEAVES={trainResult.metrics.n_leaves} | {trainResult.metrics.n_classes} CLASSES
              </p>
            </div>
          </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-terminal-black p-4 text-center">
                      ...
                      <div className="font-mono text-xs text-terminal-grid uppercase">ACCURACY</div>
                    </div>
                    <div className="bg-terminal-black p-4 text-center">
                      <div className="text-2xl font-bold text-terminal-mint">
                        {trainResult.metrics.precision.toFixed(3)}
                      </div>
                      <div className="font-mono text-xs text-terminal-grid uppercase">PRECISION</div>
                    </div>
                    <div className="bg-terminal-black p-4 text-center">
                      <div className="text-2xl font-bold text-terminal-mint">
                        {trainResult.metrics.recall.toFixed(3)}
                      </div>
                      <div className="font-mono text-xs text-terminal-grid uppercase">RECALL</div>
                    </div>
                    <div className="bg-terminal-black p-4 text-center">
                      <div className="text-2xl font-bold text-terminal-warning">
                        {trainResult.metrics.f1_score.toFixed(3)}
                      </div>
                      <div className="font-mono text-xs text-terminal-grid uppercase">F1 SCORE</div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <div className="border-2 border-terminal-black p-4">
                      <div className="text-xl font-bold text-terminal-black">
                        {trainResult.metrics.actual_depth}
                      </div>
                      <div className="font-mono text-xs text-terminal-black/70 uppercase">TREE DEPTH</div>
                    </div>
                    <div className="border-2 border-terminal-black p-4">
                      <div className="text-xl font-bold text-terminal-black">
                        {trainResult.metrics.n_leaves}
                      </div>
                      <div className="font-mono text-xs text-terminal-black/70 uppercase">LEAF NODES</div>
                    </div>
                    <div className="border-2 border-terminal-black p-4">
                      <div className="text-xl font-bold text-terminal-black">
                        {trainResult.metrics.n_classes}
                      </div>
                      <div className="font-mono text-xs text-terminal-black/70 uppercase">CLASSES</div>
                    </div>
                  </div>
                  ...
                  <div className="border-2 border-terminal-black p-4">
                    ...
                    <div className="space-y-2">
                      {Object.entries(trainResult.metrics.feature_importances)
                        .sort(([, a], [, b]) => b - a)
                        .map(([feature, importance]) => (
                          <div key={feature}>
                            <div className="flex justify-between items-center mb-1">
                              <span className="font-mono text-xs text-terminal-black">{feature}</span>
                              <span className="font-mono text-xs font-bold text-terminal-accent">
                                {(importance * 100).toFixed(1)}%
                              </span>
                            </div>
                            <div className="h-2 bg-terminal-grid">
                              <div
                  ...
                  <div className="border-2 border-terminal-black p-4">
                    ...
                    <div className="overflow-x-auto">
                      <table className="font-mono text-xs">
                        <tbody>
                          {trainResult.metrics.confusion_matrix.map((row, i) => (
                            <tr key={i}>
                              {row.map((val, j) => (
                                <td
                                  key={j}
                                  className={clsx(
                                    "w-12 h-12 text-center border border-terminal-grid",
                                    i === j ? "bg-terminal-accent/20 font-bold" : "bg-terminal-panel"
                                  )}
                                >
                                  {val}
                                </td>
                  ...
                  <div className="border-2 border-terminal-black p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(trainResult.class_labels).map(([key, label]) => (
                        <span
                          key={key}
                          className="px-3 py-1 bg-terminal-black text-terminal-mint font-mono text-xs"
                        >
                          {key}: {label}
                        </span>
                      ))}
              ...
              {activeResultTab === "predictions" && (
                <div className="overflow-x-auto border-2 border-terminal-black">
                  <table className="w-full font-mono text-xs">
                    <thead className="bg-terminal-black text-terminal-mint">
                      <tr>
                        <th className="px-4 py-2 text-left font-bold uppercase">ACTUAL</th>
                        <th className="px-4 py-2 text-left font-bold uppercase">PREDICTED</th>
                        <th className="px-4 py-2 text-left font-bold uppercase">STATUS</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-terminal-grid">
                      {trainResult.predictions.slice(0, 20).map((pred, idx) => (
                        <tr key={idx} className="hover:bg-terminal-accent/10">
                          <td className="px-4 py-2 text-terminal-black">{pred.actual_label}</td>
                          <td className="px-4 py-2 text-terminal-black">{pred.predicted_label}</td>
                          <td className="px-4 py-2">
                            {pred.actual === pred.predicted ? (
                              <span className="text-terminal-accent font-bold">CORRECT</span>
                            ) : (
